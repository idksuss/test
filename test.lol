...
local Library, Notifications = loadstring((request or http_request or http and http.request)({Url = "https://raw.githubusercontent.com/hungquan99/Arrayfield/main/wtfUI", Method = "GET"}).Body)()
local Window = Library:Window({Name = "Skull Hub | Fisch [🐟]"})
...
if not isfolder("SkullFischPC") then
    makefolder("SkullFischPC")
end

if not isfolder("SkullFischPC/Waypoints") then
    makefolder("SkullFischPC/Waypoints")
end
...
do
    ...
    do
        ...
        Section:Toggle({Name = "Auto Cast", Callback = function(state)
    ...
    do
		if (not Solara) and (not Xeno) and (not Seliware) and (not Awp) then
			...
			Section:Button({Name = "Teleport To Waypoint", Callback = function()
				...
				Services.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(toVector3(readfile("SkullFischPC/Waypoints/"..Selected_Waypoint), ","))
			end})

			Section:Button({Name = "Save Waypoint", Callback = function()
				if WayPointName == nil then return end
				writefile("SkullFischPC/Waypoints/"..WayPointName, tostring(Services.Players.LocalPlayer.Character.HumanoidRootPart.CFrame))
			end})

			Section:Button({Name = "Delete Waypoint", Callback = function()
				if Selected_Waypoint == nil then return end
				delfile("SkullFischPC/Waypoints/"..Selected_Waypoint)
			end})

			Section:Button({Name = "Refresh List", Callback = function()
				local Waypoints = {}
		
				for i,v in listfiles("SkullFischPC/Waypoints") do
					local Slash = tostring(string.gsub(v, [[SkullFischPC/Waypoints\]], ""))
	...
	do
		...
		Section:Button({Name = "TP To Appraiser", Callback = function()
			task.spawn(function()
				teleportPlayer(447.278, 150.529, 207.458)
			end)
		end})
    end
	...
	do -- Settings
		...
		Config:Keybind({Name = "Menu Key", Flag = "MenuKey", Ignore = true, UseKey = true, Default = Enum.KeyCode.RightControl, Callback = function(State) Library.UIKey = State end})
	end
end

Notifications:Notification("[🐟] Fisch | Skull Hub", 20, Library.Accent, false)
